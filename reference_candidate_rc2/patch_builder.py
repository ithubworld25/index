...
browser_new = '''    for mode in ("light", "dark", "scenario"):
        url = f"file://{(TEST / f'{mode}.html').resolve()}"
        checks = {}
        attempts = []
        text = ""
...
if browser_old not in source:
    raise SystemExit('pristine browser-validation block not found')
source = source.replace(browser_old, browser_new, 1)

# 2. Patch the injected JavaScript behavioral fixture.
replacements = {
...
for old, new in replacements.items():
    if old not in source:
        raise SystemExit(f'scenario patch marker not found: {old[:120]}')
